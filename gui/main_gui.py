...
class PHEVGUI:
    def __init__(self, root):
        ...
        tk.Label(input_frame, text="Systolic BP:", bg="#f0f4f7").grid(row=2, column=0, sticky="w", pady=5)
        self.bp_sys_var = tk.IntVar()
        tk.Entry(input_frame, textvariable=self.bp_sys_var).grid(row=2, column=1, pady=5)

        tk.Label(input_frame, text="Diastolic BP:", bg="#f0f4f7").grid(row=3, column=0, sticky="w", pady=5)
        self.bp_dia_var = tk.IntVar()
        tk.Entry(input_frame, textvariable=self.bp_dia_var).grid(row=3, column=1, pady=5)

        tk.Label(input_frame, text="Emotion (Happy/Sad/Neutral):", bg="#f0f4f7").grid(row=4, column=0, sticky="w", pady=5)
        self.emotion_var = tk.StringVar()
        tk.Entry(input_frame, textvariable=self.emotion_var).grid(row=4, column=1, pady=5)

        tk.Label(input_frame, text="Medicine Name (Optional):", bg="#f0f4f7").grid(row=5, column=0, sticky="w", pady=5)
        self.medicine_var = tk.StringVar()
        tk.Entry(input_frame, textvariable=self.medicine_var).grid(row=5, column=1, pady=5)
